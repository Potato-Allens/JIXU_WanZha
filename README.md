...
> 攻防向 13 大分类（业务与内容制作类不计入能力展示）；完整全量手册：`references/MASTER_MANUAL.md`。
...
| **硬件 / 固件 / 嵌入式** | 5g、I2C、Karma、NFC攻击、NFC重放、OTA攻击、RFID克隆、RFID攻击、SWD、binwalk、dma、dma外挂、dma攻击、ecu、fault、fpga、glitch、gsm、iot、iot固件、jtag、mips、nas、nb-iot、nfc、ota、power analysis、rfid、sdr、secure boot、side channel、spi、swarm、tpm、uart、uboot、usb、websocket、websocket抓包、侧信道、信令、功耗分析、反机器人、固件、固件更新、打印、打印机、接口、摄像头、摄像头规避、无人机、无人机劫持、无线、智能设备、木马、机器人、机器人操控、激光、点、电磁、硬件安全、硬件断点、穿戴设备、芯片、芯片卡、蓝牙、蜂窝、记录、路由器、软件无线电、近场、近场支付、固件提取(binwalk)、固件渗透链、固件布局、侧信道功耗分析、电磁侧信道、故障注入(glitch)、激光注入、SDR软件无线电、蓝牙嗅探、RFID/NFC近场、5G信令、PLC/SCADA工控、车联网V2X、ECU/OBD-II、硬件断点(DR0-3)、PCIe DMA、FPGA、JTAG/SWD调试口 |
...
```text
打包密封：
  python wz_seal.py seal --bundle wz_wangzha_1.0.zip --owner "持有方" --out dist/wz_sealed.bin
  python wz_seal.py inspect --file dist/wz_sealed.bin    # 查看防伪信息

安装：
  wz_install.exe                                          # 图形引导安装（Windows）
  python wz_install.py install --bundle wz_wangzha_1.0.zip

校验 / 回滚：
  python wz_install.py verify --bundle wz_wangzha_1.0.zip
  python wz_install.py rollback                           # 回滚到备份
  python wz_install.py status
```
...
**Q6：装坏/想退回旧版怎么办？**
覆盖安装自动备份旧版，`python wz_install.py rollback` 一键回滚；卸载还原备份。
...
3. 提供实战案例、标准打法，补充进 `references/battle_manual.md`；
